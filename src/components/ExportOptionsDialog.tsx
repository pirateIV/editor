...
export function ExportOptionsDialog({ open, onClose }: ExportOptionsDialogProps) {
   const { onExport } = useCodeStore();
   const [selectedOption, setSelectedOption] = useState<ExportOption>("multi");

   const handleExport = async (option: ExportOption) => {
      const data = await onExport(option);
      console.log(data);
      onClose();
   };
               ...
               >
                  <DialogPanel className="w-full max-w-md rounded-xl bg-white p-6 shadow-xs">
                     ...
                     <div className="flex justify-end gap-3">
                        ...
                        <Button
                           onClick={async () => await handleExport(selectedOption)}
